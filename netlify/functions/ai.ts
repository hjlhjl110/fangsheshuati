...
export const handler: Handler = async (event) => {
  try {
    if (!BASE_URL || !API_KEY || !MODEL) {
      return { statusCode: 500, body: JSON.stringify({ error: 'AI 接口未配置' }) }
    }
    ...
    if (!resp.ok) {
      ...
      return { statusCode: resp.status, body: JSON.stringify({ error: t }) }
    }
    const data = await resp.json()
    const content: string = data?.choices?.[0]?.message?.content || ''
    const parsed = parseAnswer(content)
    return { statusCode: 200, body: JSON.stringify({ answer: parsed.answer, explanation: parsed.explanation, content }) }
  } catch (e: any) {
